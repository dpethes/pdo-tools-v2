...
type
  ...
  public
    ...
    procedure InitDocProperties(const jp_font: boolean = false); override;
    procedure AddPage(const width, height: single); override;
...
const
  ...
  //lsLine, lsOutline, lsTab, lsValleyFold, lsMountainFold, lsFace
...
procedure TSvgWriter.InitDocProperties(const jp_font: boolean);
begin
  if jp_font then
     m_font_family := FontFamilyForShiftJIS;
end;

procedure TSvgWriter.AddPage(const width, height: single);
begin
...
begin
  //unused, we just copy the coords
end;
...
begin
  ...
  s := '<text';
  s += format(' x="%.3f" y="%.3f"', [x, y]);
  s += format(' style="font-size:%dpx;fill:#000000;stroke:none;font-family:%s;">',
             [font_size, m_font_family]);
  s += escaped_text + '</text>';
  m_layer.Add(s);
end;
